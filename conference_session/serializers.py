...
class SessionSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        # Obtener ID actual si se está editando
        session_id = self.instance.id if self.instance else None

        # Validación de título único (case-insensitive)
        if Session.objects.filter(title__iexact=value).exclude(id=session_id).exists():
            raise serializers.ValidationError("Ya existe una sesión con este nombre.")
        return value

    def validate(self, data):
        # --- Chairs: normalizar lista final ---
        ...
        if chairs is None and self.instance is not None:
            ...
            chairs_final = list(current_chairs_qs)
        else:
            if not chairs or len(chairs) == 0:
                raise serializers.ValidationError({"chairs": "Se requiere al menos un chair para la sesión."})
            chairs_final = list(chairs)
    ...
    def update(self, instance, validated_data):
        """
        Sobrescribir campos opcionales a None si no vienen en el request
        para que se guarden como NULL en la base de datos.
        """
        optional_fields = ['threshold_percentage', 'improvement_threshold']
